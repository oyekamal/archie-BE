...
def generate_summary(messages):
    ...
    summary_request.append(
        {
            "role": "user",
            "content": "Please summarize the content in its original language.",
        }
    )

    completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", messages=summary_request
    )
...
def activity_generate(data):
    ...
    if question:
        messages = [{"role": "system", "content": "Please create content."}]
